...
def plt_contactArea_annotated(contact_keep_sum,
    ...
    #add marker for when gastrulation is complete
    ...
    #add markers for gut cell divisions
    plt.plot([time2min(div_info.loc[embryo,'E16_lpp'], div_info.loc[embryo,'frame_rate']),
             time2min(div_info.loc[embryo,'E16_rpp'], div_info.loc[embryo,'frame_rate']),
    ...
    ax.add_patch(patches.Rectangle((time2min(div_info.loc[embryo,'int5start'], div_info.loc[embryo,'frame_rate']),
                                    ymarkerpos+2),div_info.loc[embryo,'int5len'],1.75,
    ...
    plt.title(embryo)
    plt.savefig(output_dir+'vir_gut_contact_v3_'+embryo+'.png', dpi=600, bbox_inches='tight')
