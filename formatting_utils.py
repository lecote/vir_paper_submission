...
def generate_stat_table(dc, val_list, genotype_col, ctrl, expt):
    ...
    stat_table = []
    for d in (val_list):
        n,np = stats.normaltest(dc[d], nan_policy='omit')
        s,p = stats.mannwhitneyu(dc.loc[experimental][[d]],
                dc.loc[control][[d]], nan_policy='omit')
...
def plot_puncta(dc, vars1, vars2, 
                order, stat_table, 
                figsize=(3.2,3), 
                save_pdf_file=None, x_order=None, 
                sizing=None, resizer=6):
    ...
    if sizing !=None:
        y_value_plotted = ax1.get_ylabel() #y_values assumed to be unique!!
        resizer = resizer #range of ratios 0.1-5, so 6-7 gives good size distribution
        coll = ax1.collections
        for x_val in range(0,len(coll)):
            ratio_values = []
            offsets = coll[x_val].get_offsets()
            points_y_values = coll[x_val].get_offsets().data[:,1]
            points_x_values = coll[x_val].get_offsets().data[:,0]
            for y_val in range(0,len(points_y_values)):
                r_val = dc.loc[dc[y_value_plotted] ==  points_y_values[y_val], sizing].values[0] #get "sizing" value from dataframe given y_value_plotted
                ratio_values.append(resizer*r_val) 
            coll[x_val].set_sizes(ratio_values)
            coll[x_val].set_offsets(offsets) #try to keep points in original offset position. Didn't work. Need to use offset_position='data' and replot
    ...
    plt.tight_layout()
    if save_pdf_file != None:
        plt.savefig(save_pdf_file, format='pdf')
    plt.show()
